...
// DELETE /api/users/:id — Eliminar usuario definitivamente (cascada)
// :id puede ser UUID. Alternativa: DELETE /api/users?telefono=XXX (sin :id)
router.delete("/:id", authAdmin, async (req, res, next) => {
  try {
    const result = await usersService.deleteUser({ id: req.params.id, actor: req.actorTipo });
    res.status(result.statusCode).json(result.body);
  } catch (err) {
    next(err);
  }
...
// DELETE /api/users?telefono=XXX — Eliminar por teléfono (definitivo)
router.delete("/", authAdmin, async (req, res, next) => {
  try {
    ...
    const result = await usersService.deleteUser({ telefono, actor: req.actorTipo });
    res.status(result.statusCode).json(result.body);
  } catch (err) {
    next(err);
  }
